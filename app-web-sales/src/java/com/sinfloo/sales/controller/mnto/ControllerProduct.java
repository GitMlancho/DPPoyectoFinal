            ...
            if (sessionUser != null) {
                switch (pathInfo) {
                    ...
                    case ConstantEnpoint.OBTAIN_ID:
                        ...
                        break;
                    case ConstantEnpoint.OBTAIN_CODE:
                        if (request.getMethod().equals(ConstantEnpoint.GET)) {
                            Product p=getProductDao().getObjectByCode(request.getParameter("code"));
                            if(p==null){
                                 throw new IllegalArgumentException(ConstantMessages.MESSAGE_VAL_611);
                            }
                            result.put(Constants.PRODUCT,p);
                        } else {
                            throw new AccessDeniedException(ConstantMessages.MSG_PAGE_NOT_FOUND);
                        }
                        break;
